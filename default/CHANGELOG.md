...
- **Abstract roots renamed: `Box` → `Enclosure`, `Element` → `Device`.**
  `Container` stays. Concrete enclosures (``JunctionBox``, ``DeviceBox``,
  ``Panel``, ``LightPoint``, ``OpenPort``) extend ``housewire.default.Enclosure``;
  devices extend ``housewire.default.Device`` (which extends ``Enclosure``).
- **Catalog kind `ElementType` → `DeviceType`.** Type YAMLs use
  ``kind: DeviceType`` for the device tier.
